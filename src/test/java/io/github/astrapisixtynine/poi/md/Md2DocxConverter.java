...
public class Md2DocxConverter
{
	 ...
	 */
	public static void convertMdToFormattedDocx(File mdFile, File docxFile) throws IOException
	{
		...
		System.out.println("Formatted DOCX created: " + docxFile.getAbsolutePath());
	}
	...
	private static void processMarkdownNode(XWPFDocument doc, Node node)
	{
		node.accept(new AbstractVisitor()
		{
			...
			{
				String text = getNodeText(paragraph).trim();
				if (!text.isEmpty())
			...
			public void visit(BulletList list)
			{
				...
				while (listItem != null)
				{
					if (listItem instanceof ListItem)
					{
						...
						listParagraph.setIndentationLeft(500);
						XWPFRun run = listParagraph.createRun();

						run.setText("• " + extractTextFromNode(listItem));
						run.addBreak(); // Ensures a new line is added after each list item
					}
	...
	private static String extractTextFromNode(Node node)
	{
		...
		while (child != null)
		{
			text.append(getNodeText(child)).append(" "); // Append text from child nodes
			child = child.getNext();
		}
	...
	public static void main(String[] args) throws IOException
	{
		...
		convertMdToFormattedDocx(mdFile, docxFile);
	}
}
